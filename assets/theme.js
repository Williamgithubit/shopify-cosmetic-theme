// Mobile Menu Toggle - Enhanced Modern Experience
(function () {
  ...
  mobileMenuLinks.forEach((link) => {
    link.addEventListener("click", function () {
      // Only close if it's not a current page link or if it has an href
      const href = this.getAttribute("href");
      if (href && !this.classList.contains("mobile-menu__link--active")) {
        closeMobileMenu();
      }
    });
  });
...
(function () {
  ...
  function getSelectedVariantId() {
    ...
    const variantData = document.querySelector(
...
(function () {
  ...
  function updatePrice(variant) {
    const priceElement = document.querySelector(
      ".product-main__price .price-item--regular"
    );
    const salePriceElement = document.querySelector(
      ".product-main__price .price-item--sale"
    );
    const compareAtPriceElement = document.querySelector(
      ".product-main__price .price-item--regular s"
    );
    ...
    if (priceElement) {
      ...
      if (
        ...
        if (compareAtPriceElement) {
          compareAtPriceElement.textContent = formatMoney(
            variant.compare_at_price
          );
        }
        if (salePriceElement) {
    ...
    if (variant.featured_media) {
      ...
      if (mediaItem) {
        ...
        const thumbnail = document.querySelector(
          '.product-main__thumbnail[data-media-id="' +
